...
public class EmployeeSystemLogic {
	...
	//Created By Mitchell - Not to be uploaded
	public EmployeeBean findEmployee (EmployeeBean eBean) {
		...
		*/
		...
		if (employeeList == null) {
			...
			for (EmployeeBean b : employeeList) {
				...
				if (adminFlag == 1) {
					tableBuilder.append("<td><a href=\"/EmployeeControl/EmployeeSystem?page=updateUser&lastpage&selectedUser=" + b.getEmployeeId() + "\">修正</a></td>"
							+ "    <td><a href=\"/EmployeeControl/EmployeeSystem?page=deleteUser&lastpage&selectedUser=" + b.getEmployeeId() + "\">削除</a></td>");
				}
				tableBuilder.append("<td>" + b.getEmployeeId() + "</td>"
								+ "				<td><a href=\"/EmployeeControl/EmployeeSystem?page=viewUser&selectedUser=" + b.getEmployeeId() + "\">" + b.getEmployeeName() + " </a></td>"
								+ "<td>" + b.getBaseName() + " </td>"
	...
	public ArrayList<String> confirmDeleteUser(EmployeeBean eBean) {
		ArrayList<EmployeeBean> employeeList;
		EmployeeSystemDAO eSysDAO = new EmployeeSystemDAO();
		ArrayList<String> message = new ArrayList<String>();
		...
		if (employeeList == null) {
			...
			output.append("<h1 style=\"text-align:left\">削除内容確認</h1>" +
			"<p> 社員ID： " + eBean.getEmployeeId() + "" +
					"<p> パスワード： " + eBean.getPassword() + "" +
					"<p> 名前： " + eBean.getEmployeeName() + "" +
					"<p> ふりがな： " + eBean.getKana() + "" +
					"<p>性別： " + eBean.getGender() + "" +
					"<p>部署名： " + eBean.getDepartmentName() + "" +
					"<p>課名： " + eBean.getDivisionName() + "" +
					"<p>役職名： " + eBean.getPositionName() + "" +
					"<p>役職詳細： " + eBean.getPositionMemo() + "" +
					"<p>内線： " + eBean.getNaisenNumber() + "" +
					"<p>業務携帯番号：" + eBean.getPublicCellphoneNumber() + "" +
					"<p> 写真：<img src=\"img/" + eBean.getEmployeeId() + ".jpg\"/>");
			message.add(output.toString());
		}
//		e016	社員IDが存在しません
//		e017	データベースでエラーが発生しています
		return message;
	}

	public ArrayList<String> delete (EmployeeBean eBean) {
		ArrayList<String> message = new ArrayList<String>();
		EmployeeSystemDAO eSysDAO = new EmployeeSystemDAO();
		int result;

		String sql = "DELETE FROM employee WHERE employeeID = " + eBean.getEmployeeId() + ";";

		result = eSysDAO.updateEmployee(sql);
		if(result > 0) {
			message.add("m003:削除できました");
		} else {
			message.add("e016:社員IDが存在しません");
		}
